                ...
                <Stack spacing={2.5}>
                    ...
                    <Surface>
                        <AppTable
                            ...
                            columns={[
                                { id: 'publicId', label: 'Incident', priority: 'primary', render: (row) => row.publicId },
                                { id: 'title', label: 'Title', priority: 'primary', render: (row) => row.title },
                                { id: 'status', label: 'Notification assessment', priority: 'primary', render: (row) => row.notificationStatus },
                                { id: 'owner', label: 'Owner', priority: 'primary', render: (row) => row.owner },
                                { id: 'activities', label: 'Affected activity', priority: 'primary', render: (row) => (row.activities || []).join(', ') || 'Not linked' },
                                { id: 'risk', label: 'Risk', priority: 'primary', render: (row) => row.privacyRisk },
                                { id: 'dates', label: 'Date', priority: 'primary', render: (row) => [row.occurredAt ? `Occurred ${new Date(row.occurredAt).toLocaleDateString()}` : null, row.closedAt ? `Closed ${new Date(row.closedAt).toLocaleDateString()}` : null].filter(Boolean).join(' · ') || 'Not recorded' },
                                { id: 'incident', label: 'Linked incident', priority: 'secondary', render: (row) => row.incident },
                                { id: 'data', label: 'Data', priority: 'secondary', render: (row) => (row.dataCategories || []).join(', ') || 'Not recorded' },
                                { id: 'subjects', label: 'Subjects', priority: 'secondary', render: (row) => (row.dataSubjects || []).join(', ') || 'Not recorded' },
                                { id: 'systems', label: 'Systems', priority: 'secondary', render: (row) => (row.systems || []).join(', ') || 'Not recorded' },
                                { id: 'vendors', label: 'Vendors', priority: 'secondary', render: (row) => (row.vendors || []).join(', ') || 'Not recorded' },
                                { id: 'jurisdictions', label: 'Jurisdictions', priority: 'secondary', render: (row) => (row.jurisdictions || []).join(', ') || 'Not recorded' },
                                { id: 'findings', label: 'Findings', priority: 'secondary', render: (row) => row.findings },
                                { id: 'decision', label: 'Decision', priority: 'secondary', render: (row) => row.decision },
                                { id: 'decide', label: 'Record decision', priority: 'secondary', render: (row) => (
