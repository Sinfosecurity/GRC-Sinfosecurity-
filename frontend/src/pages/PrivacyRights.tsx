                ...
                <Stack spacing={2.5}>
                    <Surface>
                        ...
                        <Stack component="form" onSubmit={create} direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
                            <TextField select label="Request type" value={requestType} onChange={(event) => setRequestType(event.target.value)} sx={{ minWidth: 180 }}>
                                {['ACCESS', 'CORRECTION', 'DELETION', 'RESTRICTION', 'OBJECTION', 'PORTABILITY', 'OPT_OUT', 'CONSENT_WITHDRAWAL'].map((item) => <MenuItem key={item} value={item}>{item.replace(/_/g, ' ')}</MenuItem>)}
                            </TextField>
                            <TextField select label="Regime" value={regime} onChange={(event) => setRegime(event.target.value)} sx={{ minWidth: 160 }}>
                                {['GDPR', 'UK_GDPR', 'CCPA_CPRA', 'US_STATE'].map((item) => <MenuItem key={item} value={item}>{item.replace(/_/g, ' ')}</MenuItem>)}
                            </TextField>
                    ...
                    {selected && (
                        <Surface>
                            ...
                            <Typography>Verification: {selected.verificationStatus}</Typography>
                            ...
                            <Stack direction="row" spacing={1} sx={{ mt: 1.5 }} flexWrap="wrap" useFlexGap>
                                <Button onClick={() => privacyAPI.updateRights(selected.publicId, { verificationStatus: 'PENDING' }).then((res) => setSelected(res.data.data))}>Mark verification pending</Button>
