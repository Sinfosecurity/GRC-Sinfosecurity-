...
import { Alert, Box, Button, ListItemButton, Stack, Typography } from '@mui/material';
                ...
                {data && (
                    <Stack spacing={2.5}>
                        ...
                        <Surface>
                            ...
                            <Stack spacing={0.5}>
                                {data.attention.map((row) => (
                                    <ListItemButton key={`${row.type}-${row.publicId}`} onClick={() => navigate(row.href)} aria-label={`Open ${humanizeLabel(row.type)}`}>
                                        <Box>
                        ...
                        <AttentionStrip items={[
                            { label: 'Active processing', value: data.totals.activeActivities },
                            { label: 'Open rights requests', value: data.totals.openRightsRequests },
                            { label: 'Transfers in review', value: data.totals.transfersRequiringReview },
                            { label: 'Retention due', value: data.totals.retentionActionsDue },
                        ]} />
                        <Surface>
