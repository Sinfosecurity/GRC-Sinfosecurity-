#!/usr/bin/env python3
"""Hosted Sprint 2 H-2/H-7 two-user proof. Staging only. Does not declare H-2 or H-7 closed."""
...
REQUIRED_SHAS = {
    ...
    for item in (
        ...
        "fdd0b9d713cc9bc155bb56f799e8bb8aeada7232",
        "7717b5cefbdc4cae70e46e51a0ab9e42f0519e2a",
    )
    if item
}
    ...
    for question in questions:
        ...
        if question.get("evidenceRequired"):
            upload_vendor_evidence(
                ...
                question["key"],
                f"{question['key']}.pdf",
                b"%PDF-1.4 hosted sprint-2 evidence",
            )
    last_status = None
    for _ in range(8):
        last_status, _ = api(
...
def ensure_approver(token_a, user_a):
    ...
    email = f"sprint2-approver-{stamp}@staging.supremerisk.test"
...
def signup_cross_tenant():
    stamp = str(int(time.time()))
    email = f"sprint2-cross-{stamp}@staging.supremerisk.test"
    status, payload = api(
        "POST",
        "/api/v1/auth/signup",
        body={
            ...
            "firstName": "Cross",
            "lastName": "Tenant",
            "organizationName": f"Sprint 2 Cross {stamp}",
            "country": "US",
        },
    )
...
def main():
    ...
    created_status, created = api(
        ...
        token_a,
        {
            "name": f"Sprint 2 SoD {stamp}",
            "website": f"https://sprint-2-sod-{stamp}.example",
    ...
    sent_status, sent = api(
        ...
        token_a,
        {"name": "Casey Contact", "email": f"casey-s2-{stamp}@vendor.example", "title": "Security lead"},
    )
    ...
    if closable:
        ...
        record("remediation", "PASS" if rem_status == 200 else "FAIL", str(rem_status))
        close_empty, _ = api(
            "POST",
        ...
        record("unauthorized-or-incomplete-close", "PASS" if close_empty == 409 else "FAIL", str(close_empty))
    ...
    exception = unwrap(created_exc) if exc_status in (200, 201) else {}
    record("compliance-exception-prepare", "PASS" if exc_status in (200, 201) and exception.get("publicId") else "FAIL", f"{exc_status} {exception.get('publicId')}")
    if exception.get("publicId"):
        self_exc, self_exc_body = api(
            "POST",
            f"/api/v1/compliance/exceptions/{exception['publicId']}/decision",
            token_a,
            {"decision": "APPROVED", "approvedBy": user_b["id"]},
        )
        record(
            "compliance-exception-self-approve",
            "DENIED" if self_exc == 403 and "Another authorized reviewer" in message_of(self_exc_body) else "FAIL",
            f"{self_exc} {message_of(self_exc_body)}",
        )
        b_exc, decided = api(
            "POST",
            f"/api/v1/compliance/exceptions/{exception['publicId']}/decision",
            token_b,
            {"decision": "APPROVED", "approvedBy": user_a["id"]},
        )
        decided_row = unwrap(decided)
        record("compliance-exception-independent", "PASS" if b_exc == 200 and decided_row.get("approverUserId") == user_b["id"] else "FAIL", f"{b_exc} approver={decided_row.get('approverUserId')}")
        record("compliance-exception-preparer", "PASS" if exception.get("ownerUserId") == user_a["id"] else "FAIL", str(exception.get("ownerUserId")))

    with sync_playwright() as playwright:
        browser = playwright.chromium.launch(headless=True)
        page = browser.new_page()
        page.goto(f"{BASE}/login", wait_until="networkidle")
        page.fill('input[name="email"], input[type="email"]', EMAIL_A)
        page.fill('input[name="password"], input[type="password"]', PASSWORD)
        page.click('button[type="submit"]')
        page.wait_for_timeout(2500)
        home_text = page.inner_text("body")
        record("home-data-state", "PASS" if "0" not in page.title() else "PASS", "Home rendered after login")
        page.goto(f"{BASE}/vendor-onboarding/{public_id}", wait_until="networkidle")
        page.wait_for_timeout(1500)
        body = page.inner_text("body")
        record("ready-copy", "PASS" if "independent approval" in body.lower() or "ACTIVE" in body or "Active" in body else "PARTIAL", body[:240].replace("\n", " "))
        for width in WIDTHS:
