#!/usr/bin/env python3
"""Hosted #12-V final experience closure: Home data-state + Review & Decide. Staging only. Does not declare #12 PASS."""
...
OUT = ROOT / "docs" / "private-beta" / "hosted-ux-qa" / os.environ.get("E2E_EVIDENCE", "supreme-experience-2-final-closure")
...
def false_all_clear(page):
    text = page.inner_text("body")
    return "Nothing needs your attention" in text or "0 need your attention" in text


def prove_home(page, context, token):
    held = []

    def hold_home(route):
        url = route.request.url
        if any(part in url for part in ("/tprm/attention", "/vendors/statistics", "/intelligence/teaser")):
            held.append(route)
            return
        route.continue_()

    context.route("**/api/v1/**", hold_home)
    page.goto(f"{BASE}/dashboard", wait_until="domcontentloaded")
    page.wait_for_timeout(400)
    loading = "Checking what needs your attention" in page.inner_text("body")
    record("home-loading-throttled-visible", "PASS" if loading else "FAIL", "pending home APIs held")
    record("home-throttled-no-all-clear", "FAIL" if false_all_clear(page) else "PASS", "throttled pending")
    for width in WIDTHS:
        shot(page, "home-loading-throttled", width)
    run_axe(page, "home-loading-throttled")
    for route in held:
        route.continue_()
    context.unroute("**/api/v1/**", hold_home)
    page.wait_for_timeout(1800)
    after_hold = page.inner_text("body")
    record("home-throttled-then-data", "PASS" if "Checking what needs your attention" not in after_hold and "need your attention" in after_hold else "PARTIAL", "held routes released")

    page.goto(f"{BASE}/dashboard", wait_until="commit")
    page.wait_for_timeout(150)
    if "Checking what needs your attention" in page.inner_text("body"):
        record("home-loading-normal-visible", "PASS", "hard refresh showed checking state")
        record("home-refresh-no-all-clear", "FAIL" if false_all_clear(page) else "PASS", "hard refresh pending")
        for width in WIDTHS:
            shot(page, "home-loading-normal", width)
    else:
        record("home-loading-normal-visible", "PARTIAL", "refresh completed before checking copy was observable; throttled shots remain the held-request proof")
        shot(page, "home-loading-normal", 1440)
    page.wait_for_load_state("networkidle")
    page.wait_for_timeout(700)
    live = api("GET", "/api/v1/tprm/attention", token)[1]
    live_count = len((live.get("data") or {}).get("items") or [])
    RESULTS["home"]["attentionItems"] = live_count
    RESULTS["home"]["work"] = (live.get("data") or {}).get("work")
    body = page.inner_text("body")
    record("home-loaded-real-data", "PASS" if f"{live_count} need your attention" in body else "PARTIAL", f"ui vs api {live_count}")
    for width in WIDTHS:
        shot(page, "home-loaded-real-data", width)
    run_axe(page, "home-loaded")

    def zero_attention(route):
        route.fulfill(status=200, content_type="application/json", body=json.dumps({
            "success": True,
            "data": {"items": [], "work": {"dueAssessments": 0, "overdueFindings": 0, "pendingDecisions": 0}},
        }))

    context.route("**/api/v1/tprm/attention*", zero_attention)
    page.reload(wait_until="networkidle")
    page.wait_for_timeout(600)
    record("home-true-zero", "PASS" if "Nothing needs your attention" in page.inner_text("body") else "FAIL", "successful empty attention")
    shot(page, "home-true-zero", 1440)
    context.unroute("**/api/v1/tprm/attention*", zero_attention)

    def fail_stats(route):
        if "/vendors/statistics" in route.request.url:
            route.fulfill(status=500, content_type="application/json", body=json.dumps({"success": False, "message": "Statistics request failed"}))
            return
        route.continue_()

    context.route("**/api/v1/vendors/statistics*", fail_stats)
    page.reload(wait_until="networkidle")
    page.wait_for_timeout(700)
    text = page.inner_text("body")
    record("home-partial-stats-error", "PASS" if "unavailable" in text.lower() and "Nothing needs your attention" not in text else "PARTIAL", "stats failed, attention must remain truthful")
    shot(page, "home-error-or-partial-state", 1440)
    context.unroute("**/api/v1/vendors/statistics*", fail_stats)
