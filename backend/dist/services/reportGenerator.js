...
class ReportGenerator {
     ...
     */
    async generateReport(templateId, format = 'pdf', generatedBy = 'system') {
        const template = templates.get(templateId);
        if (!template) {
            throw new Error('Template not found');
        }
        // Collect data for each section
        const sectionData = await Promise.all(template.sections.map(section => this.getSectionData(section)));
        const report = {
            id: `rpt_${Date.now()}`,
            templateId,
