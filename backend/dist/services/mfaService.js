...
class MFAService {
    ...
    generateTOTPSecret() {
        return crypto_1.default.randomBytes(20).toString('base32').replace(/=/g, '');
    }
    ...
    generateBackupCodes() {
        return Array.from({ length: this.BACKUP_CODES_COUNT }, () => crypto_1.default.randomBytes(4).toString('hex').toUpperCase());
    }
    ...
    generateChallengeId() {
        return `mfa_${Date.now()}_${crypto_1.default.randomBytes(8).toString('hex')}`;
    }
    hashCode(code) {
    ...
    generateTOTPCode(secret, time) {
        ...
        const hmac = crypto_1.default.createHmac('sha1', Buffer.from(secret, 'base32'));
        hmac.update(buffer);
    ...
    storeChallenge(userId, method, code) {
        const challengeKey = `${userId}-${method}`;
...
exports.default = new MFAService();
