...
class VendorContractService {
    ...
    async createContract(data) {
        const contract = await database_1.prisma.vendorContract.create({
            data: {
                ...data,
        ...
        logger_1.default.info(`✅ Created contract: ${contract.title} for ${contract.vendor.name}`);
        return contract;
    }
    /**
     * Get contract by ID
    ...
    async getContractById(contractId, organizationId) {
        return await database_1.prisma.vendorContract.findFirst({
            where: {
                id: contractId,
                organizationId,
            },
     ...
     */
    async listVendorContracts(vendorId, organizationId) {
        return await database_1.prisma.vendorContract.findMany({
            where: {
                vendorId,
                organizationId,
            },
     ...
     * Update contract
     */
    async updateContract(contractId, organizationId, data) {
        return await database_1.prisma.vendorContract.updateMany({
            where: {
                id: contractId,
                organizationId,
            },
     ...
     * Approve contract
     */
    async approveContract(contractId, organizationId, approvedBy) {
        return await database_1.prisma.vendorContract.updateMany({
            where: {
                id: contractId,
                organizationId,
     ...
     */
    async getExpiringContracts(organizationId, withinDays = 90) {
        const futureDate = new Date();
        futureDate.setDate(futureDate.getDate() + withinDays);
        const contracts = await database_1.prisma.vendorContract.findMany({
            where: {
                organizationId,
                status: client_1.ContractStatus.ACTIVE,
                expirationDate: {
        ...
        for (const contract of contracts) {
            const daysUntilExpiry = Math.floor((contract.expirationDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
            if (daysUntilExpiry <= 30 && contract.status !== client_1.ContractStatus.EXPIRING_SOON) {
                await database_1.prisma.vendorContract.update({
                    where: { id: contract.id },
                    data: { status: client_1.ContractStatus.EXPIRING_SOON },
                });
            }
        }
        return contracts;
    ...
    async trackSLAMetric(data) {
        ...
        const slaRecord = await database_1.prisma.sLATracking.create({
            data: {
                ...data,
                status,
        ...
        if (status === 'BREACHED') {
            const contract = await database_1.prisma.vendorContract.findUnique({
        ...
        logger_1.default.info(`✅ SLA tracked: ${data.metricName} - ${status}`);
        return slaRecord;
    }
    /**
     * Get SLA compliance report
     */
    async getSLAComplianceReport(organizationId, period) {
        const startDate = this.getPeriodStartDate(period);
        const slaRecords = await database_1.prisma.sLATracking.findMany({
            where: {
                contract: {
                    organizationId,
                },
        ...
        // Group by vendor
        const byVendor = slaRecords.reduce((acc, record) => {
     ...
     */
    async getContractStatistics(organizationId) {
        const [totalContracts, activeContracts, expiringContracts, totalValue, contractsByType,] = await Promise.all([
            database_1.prisma.vendorContract.count({ where: { organizationId } }),
            database_1.prisma.vendorContract.count({
                where: { organizationId, status: client_1.ContractStatus.ACTIVE },
            }),
            database_1.prisma.vendorContract.count({
                where: {
                    organizationId,
            ...
            database_1.prisma.vendorContract.aggregate({
                where: { organizationId, status: client_1.ContractStatus.ACTIVE },
                _sum: { contractValue: true },
            }),
            database_1.prisma.vendorContract.groupBy({
                by: ['contractType'],
                where: { organizationId, status: client_1.ContractStatus.ACTIVE },
                _count: true,
            }),
...
exports.default = new VendorContractService();
//# sourceMappingURL=vendorContractService.js.map