...
class EmailService {
    ...
    async send(options) {
        ...
        if (this.isDevelopment || !this.apiKey) {
            ...
            logger_1.default.info('═══════════════════════════════════════');
            logger_1.default.info(`From: ${from}`);
            logger_1.default.info(`To: ${recipients.join(', ')}`);
            logger_1.default.info(`Subject: ${options.subject}`);
            logger_1.default.info('───────────────────────────────────────');
            logger_1.default.info(options.text || this.htmlToText(options.html));
            logger_1.default.info('═══════════════════════════════════════\n');
            return true;
        }
        try {
            ...
            logger_1.default.info(`✅ Email sent successfully to ${recipients.join(', ')}`);
            return true;
        }
        catch (error) {
    ...
    async sendTemplate(templateName, to, data) {
        ...
        if (!template) {
            logger_1.default.error(`Template "${templateName}" not found`);
            return false;
        }
        return this.send({
            to,
