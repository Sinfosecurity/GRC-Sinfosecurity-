...
class VendorRiskHistoryService {
    ...
    async recordRiskSnapshot(
        ...
        try {
            ...
            const criticalIssuesCount = vendor.issues.filter(
            ...
            await prisma.vendorRiskHistory.create({
                data: {
                    vendorId,
                    organizationId,
            ...
            logger.info('Risk history snapshot recorded', {
                vendorId,
                residualRiskScore: vendor.residualRiskScore,
                changeReason,
            });
    ...
    async getVendorRiskTrend(vendorId: string, organizationId: string, months: number = 12) {
        try {
            ...
            const history = await prisma.vendorRiskHistory.findMany({
                where: {
                    vendorId,
                    organizationId,
            ...
            logger.error('Failed to get vendor risk trend', { error: error.message, vendorId });
            throw error;
        }
    }
     ...
     */
    async getAllVendorTrends(organizationId: string, months: number = 6) {
        try {
            const startDate = new Date();
            startDate.setMonth(startDate.getMonth() - months);
            ...
            logger.error('Failed to get all vendor trends', { error: error.message, organizationId });
            throw error;
        }
    }
     ...
     */
    private generateInsights(history: any[], trend: any): string[] {
        ...
        const issuesIncreasing = recentIssues.every((h, i) => 
            i === 0 || h.openIssuesCount >= recentIssues[i - 1].openIssuesCount
        );
     ...
     */
    private determineAlertLevel(currentScore: number, trend: any): string {
...
export default new VendorRiskHistoryService();
