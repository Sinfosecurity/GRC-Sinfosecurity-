...
interface ConcentrationRiskAnalysis {
    ...
    recommendations: string[];
    overallRiskRating: string; // "Low", "Medium", "High", "Critical"
}
...
class VendorConcentrationRiskService {
     ...
     */
    async analyzeConcentrationRisk(organizationId: string): Promise<ConcentrationRiskAnalysis> {
        try {
            ...
            const overallRiskRating = this.calculateOverallRiskRating(
                spendConcentration,
                geographicConcentration,
                categoryConcentration,
            ...
            logger.info('Concentration risk analysis completed', {
                organizationId,
                totalVendors,
                criticalVendors,
                overallRiskRating,
            ...
            logger.error('Failed to analyze concentration risk', { error: error.message, organizationId });
            throw error;
        }
    }
     ...
     * Analyze spend concentration
     */
    private analyzeSpendConcentration(vendors: any[], totalSpend: number) {
     ...
     * Analyze geographic concentration
     */
    private analyzeGeographicConcentration(vendors: any[]) {
        const countryMap = new Map<string, { total: number; critical: number }>();

        vendors.forEach(v => {
            if (v.geographicFootprint && Array.isArray(v.geographicFootprint)) {
                v.geographicFootprint.forEach((country: string) => {
                    const existing = countryMap.get(country) || { total: 0, critical: 0 };
                    existing.total++;
                    if (v.tier === 'CRITICAL') existing.critical++;
     ...
     * Analyze category concentration
     */
    private analyzeCategoryConcentration(vendors: any[], totalSpend: number) {
        const categoryMap = new Map<string, { count: number; critical: number; spend: number }>();
     ...
     * Identify single points of failure
     */
    private identifySinglePointsOfFailure(vendors: any[]) {
        const criticalVendors = vendors.filter(v => 
            v.tier === 'CRITICAL' || v.criticalityLevel === 'CRITICAL'
        );
     ...
     */
    private generateMitigationRecommendation(vendor: any): string {
        ...
        if (vendor.dataTypesAccessed?.some((dt: string) => ['PII', 'PHI', 'PCI'].includes(dt))) {
    ...
    private generateRecommendations(
        spendConcentration: any,
        geographicConcentration: any,
        categoryConcentration: any,
        singlePointsOfFailure: any[]
    ): string[] {
        const recommendations: string[] = [];
        ...
        if (spendConcentration.top1Vendor && spendConcentration.top1Vendor.percentOfTotal > 40) {
            recommendations.push(
                `CRITICAL: Single vendor (${spendConcentration.top1Vendor.vendorName}) represents ${Math.round(spendConcentration.top1Vendor.percentOfTotal)}% of total spend - consider diversification`
            );
        }

        if (spendConcentration.top3Percent > 60) {
            recommendations.push(
                `Top 3 vendors represent ${Math.round(spendConcentration.top3Percent)}% of spend - implement vendor diversification strategy`
            );
        }
        ...
        if (categoryConcentration.highestRiskCategory && categoryConcentration.highestRiskCategory.criticalCount > 5) {
            recommendations.push(
                `${categoryConcentration.highestRiskCategory.criticalCount} critical vendors in ${categoryConcentration.highestRiskCategory.category} - conduct detailed category risk assessment`
            );
        }
        ...
        if (singlePointsOfFailure.length > 0) {
            recommendations.push(
                `${singlePointsOfFailure.length} critical vendors without documented backup solutions - develop business continuity plans`
            );
        }
    ...
    async exportBoardReport(organizationId: string) {
        try {
            ...
            logger.error('Failed to export board report', { error: error.message, organizationId });
            throw error;
        }
    }
}

export default new VendorConcentrationRiskService();
