...
export async function sendDueDiligence(organizationId: string, vendorKey: string, actor: Actor, input: {
    ...
    if (vendor.onboarding!.stage !== VendorOnboardingStage.READY_TO_SEND && vendor.onboarding!.stage !== VendorOnboardingStage.AWAITING_VENDOR) {
        throw new ApiError(409, 'The due-diligence plan must be confirmed before it is sent.');
    }
    ...
    await prisma.vendorAssessmentInvitation.updateMany({
        where: { organizationId, vendorId: vendor.id, status: VendorInvitationStatus.PENDING },
    ...
    if (vendor.businessOwnerUserId) {
        await notifyUser({
            ...
            title: 'Due diligence sent',
            body: `${vendor.name} has been invited to complete assigned assessments.`,
            resourceType: 'Vendor',
...
export async function activationLink(organizationId: string, vendorKey: string, actor: Actor) {
    if (!canSend(actor.role)) throw new ApiError(403, 'Only a risk reviewer can copy the activation link.');
    const sent = await resendInvitation(organizationId, vendorKey, actor);
    return { activationUrl: sent.activationUrl, invitation: sent.invitation };
}
...
export async function submitVendorAssessment(actor: VendorActor, assessmentId: string, input: { attested?: boolean }) {
    ...
    if (vendor?.businessOwnerUserId) {
        await notifyUser({
            ...
            title: 'Vendor assessment submitted',
            body: `${vendor.name} submitted ${detail.name}.`,
            resourceType: 'Vendor',
...
export async function analystReview(organizationId: string, vendorId: string) {
    ...
    for (const assessment of assessments) {
        ...
        for (const row of assessment.responses) {
            ...
            const exception = finding || (score != null && score <= 5 && !String(row.response || '').toLowerCase().startsWith('yes'));
...
export async function scanVendorDueDiligenceAttention(now = new Date()) {
    const rows = await prisma.vendorOnboarding.findMany({
        ...
        include: { vendor: { select: { id: true, name: true, businessOwnerUserId: true, organizationId: true } } },
        take: 50,
    });
    let sent = 0;
    for (const row of rows) {
        ...
        await notifyUser({
            ...
            body: `${row.vendor.name} has not submitted assigned due diligence.`,
            resourceType: 'Vendor',
