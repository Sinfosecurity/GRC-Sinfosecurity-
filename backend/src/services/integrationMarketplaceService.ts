...
class IntegrationMarketplaceService {
    ...
    installIntegration(integrationId: string): boolean {
        ...
        if (!integration || integration.status !== 'available') return false;
     ...
     */
    testConnection(integrationId: string): { success: boolean; message: string } {
        const integration = integrations.get(integrationId);
        if (!integration) {
...
export default new IntegrationMarketplaceService();
