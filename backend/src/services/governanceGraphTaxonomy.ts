...
export const CURRENT_TPRM_NODE_TYPES: GovernanceNodeType[] = [
    ...
    GovernanceNodeType.MONITORING_PROFILE,
    GovernanceNodeType.MONITORING_SIGNAL,
    GovernanceNodeType.REASSESSMENT,
    GovernanceNodeType.OFFBOARDING,
...
export const SOURCE_RECORD_HREF: Record<string, (sourceId: string, extras?: { vendorId?: string }) => string> = {
    ...
    EngagementMonitoringProfile: (id) => `/engagements/${id}/monitoring`,
    EngagementMonitoringSignal: (id) => `/monitoring/signals/${id}`,
    EngagementReassessment: (id) => `/engagements/${id}/reassessment`,
    EngagementOffboardingCase: () => '/offboarding',
    EngagementFinalDisposition: () => '/offboarding',
