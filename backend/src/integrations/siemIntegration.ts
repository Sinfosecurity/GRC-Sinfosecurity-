...
class SIEMIntegration {
     ...
     * Send event to SIEM
     */
    async sendEvent(event: SecurityEvent): Promise<boolean> {
        try {
     ...
     */
    private formatEventForProvider(event: SecurityEvent): any {
        const baseEvent = {
     ...
     * Log risk event
     */
    async logRiskEvent(risk: any, action: string): Promise<boolean> {
     ...
     * Log incident
     */
    async logIncident(incident: any): Promise<boolean> {
        return this.sendEvent({
            timestamp: new Date(),
     ...
     */
    async logComplianceViolation(framework: string, violation: any): Promise<boolean> {
        return this.sendEvent({
            timestamp: new Date(),
     ...
     * Batch export logs
     */
    async batchExportLogs(events: SecurityEvent[]): Promise<number> {
        let successCount = 0;
