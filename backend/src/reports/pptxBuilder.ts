...
function slideXml(title: string, bullets: string[], kpis: Array<{ label: string; value: string }> = [], footnote?: string): string {
    ...
    const lines = [title, ...bullets.slice(0, 8).map((item) => `• ${item}`), ...(footnote ? [footnote] : [])].map((line, index) => {
      ...
      <p:spPr>
        <a:xfrm><a:off x="457200" y="${y}"/><a:ext cx="8229600" cy="457200"/></a:xfrm>
        <a:prstGeom prst="rect"><a:avLst/></a:prstGeom>
      </p:spPr>
      <p:txBody>
        <a:bodyPr/><a:lstStyle/>
        <a:p><a:r><a:rPr lang="en-US" sz="${index === 0 ? 2800 : 1600}" b="${index === 0 ? 1 : 0}" dirty="0">
          <a:solidFill><a:srgbClr val="${index === 0 ? '0F172A' : '1E293B'}"/></a:solidFill>
        </a:rPr><a:t>${xml(line.slice(0, 180))}</a:t></a:r></a:p>
      </p:txBody>
    </p:sp>`;
    }).join('');
